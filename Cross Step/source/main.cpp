...
void Load()
{
	// Iterate over the array
	{
		...
		for (int i = 0; i < 10; ++i)
		{
			...
			enemy1.Draw();
			banditVector[i].enemyY -= screenHeight*.02*GetDeltaTime();
			banditVector[i].enemyX = screenWidth*.5;
			banditVector[i].enemyY = screenHeight*.9;
			banditVector[i].enemyHealth = 1;
			banditVector.push_back(enemy1);
			enemy1.enemySprite = CreateSprite("./images/enemy1.png", banditVector[i].enemyWidth, banditVector[i].enemyHeight, true);
	...
	do
	{
		...
		case GAMEPLAY:
			...
			for (int i = 0; i < 10; ++i)
			{
				...
				banditVector[i].enemyX += screenWidth*.2;
				if (banditVector[i].enemyX > screenWidth)
				{
					banditVector[i].enemyX = 0;
				}
